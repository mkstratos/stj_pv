...
class Plotting(object):
    ...
    def poly_2PV_line(self, hemi, u_zonal, lat_elem, time_loop, pause, click):
        ...
        line6 = ax.plot(self.phi_2PV, self.theta_2PV, linestyle='-', c='k',
                        marker='x', markersize=8, label='dynamical tropopause')
        line7 = ax.plot(self.lat, self.TropH_theta[time_loop, :], linestyle='-', c='k',
                        marker='.', markersize=4, label='thermodynamic tropopause')
        line8 = ax.plot(self.cross_lat, self.cross_lev, linestyle=' ', marker='x',
                        markersize=10, mew=2, c='#009900', label='tropopause crossing')
        line9 = ax.plot(self.STJ_lat, self.jet_max_theta, linestyle=' ', marker='o',
                        c='#ff3300', markersize=16, markeredgecolor='none',
                        label='Subtropical jet')
        line10 = ax.plot(self.phi_2PV, self.pv_fit, linestyle='-',
                         linewidth=1, c='yellow', label='Poly fit')
        ...
        line1 = ax3.plot(self.dy, self.dxdy, linestyle='-', linewidth=1,
                         c='#0033ff', label=r'$\frac{d \theta}{d \phi}$')
        line2 = ax3.plot(self.dy[self.local_elem], self.dxdy[self.local_elem],
                         linestyle=' ', mew=2, c='#0033ff', marker='x', markersize=12,
                         label=r'peaks')
        ...
        loc = (0.65, 0.925)
        legend = ax3.legend(lines, labels, loc=loc, fontsize=14,
        ...
        labels = [l.get_label() for l in lines]
        loc = (0.65, 0.8)
        legend = ax.legend(lines, labels, loc=loc, fontsize=14,
                           frameon=False, numpoints=1)
        # set marker size in legend
        legend.legendHandles[2]._legmarker.set_markersize(
            8)  # crossing marker size in legend
...
def plot_u(fname):
    ...
    for t in range(len(t_elem)):
        ...
        jet_SH = [-32, -28.8, -28.6, -28.6, -28.4, -
                  31.4, -33.2, -33.4, -27.8, -20.4, -42.6]
        ...
        draw_map_model(plt, ax1, ax_cb, uwnd, var['lon'], var['lat'], '', '', 'BuRd',
                       bounds, fname_out, True, domain=None, name_cbar=None,
                       coastline=True)
