...
class Generate_IPV_Data(object):
    ...
    def __init__(self, Exp):
        ...
        self.test_daily = Exp.test_daily
    ...
    def OpenFile(self):
        ...
        # restruct data in time from first month to last month. See function
        # PathFilenameERA
...
class STJ_Post_Processing(object):
    ...
    def PlotIPV(self, output_plotting):
        ...
        fig1 = plot_map(lon_in=self.lon, lat_in=self.lat, colour='BuRd',
                        bounds=np.arange(-10., 10.5, 1.0), model_type='Mima',
                        data=self.ipv_310[0, :, :], cbar_units='PVU', filename=filename,
                        show_plot=False)
        ...
        ax1.pcolormesh(lat_array, th_levels_trop,
                       output_plotting['NH', 'ipv_zonal_0'], cmap=cmap, norm=norm)
        plt.ylim(300, 400)
        ax1.set_xlim(-90, 90)
        ax_cb = fig2.add_axes([0.1, 0.1, 0.80, 0.05])
        cbar = mpl.colorbar.ColorbarBase(
            ax_cb, cmap=cmap, norm=norm, ticks=bounds, orientation='horizontal')
        cs1 = ax1.contour(lat_array, theta_lev_array,
                          output_plotting['NH', 'ipv_zonal_0'], levels=np.arange(2, 3, 1),
                          colors='red')
        cs2 = ax1.contour(lat_array, theta_lev_array,
                          output_plotting['NH', 'ipv_zonal_0'],
        ...
        plt.ylim(250, 400)
        CS = plt.contour(lat_array, theta_lev_array, output_plotting['NH', 'ipv_zonal_0'],
                         levels=np.arange(-40, 40, 1), colors='k')
        ...
        plt.plot(output_plotting['NH', 'phi_2PV'][0, :],
                 output_plotting['NH', 'theta_2PV'][0, :], marker='x', c='k',
                 linestyle=' ', markersize=10)
        plt.plot(output_plotting['SH', 'phi_2PV'][0, :],
                 output_plotting['SH', 'theta_2PV'][0, :], marker='x', c='k',
                 linestyle=' ', markersize=10)
        # mark the STJ
        plt.plot(output_plotting['NH', 'dThdlat_lat'][0],
                 output_plotting['NH', 'dThdlat_theta'][0], marker='x', c='blue',
                 linestyle=' ', markersize=16)
        plt.plot(output_plotting['SH', 'dThdlat_lat'][0],
                 output_plotting['SH', 'dThdlat_theta'][0], marker='x', c='blue',
                 linestyle=' ', markersize=16)
        ...
        norm = mpl.colors.BoundaryNorm(bounds, cmap.N)
        ax1.pcolormesh(self.lat_NH_array, self.theta_domain_array,
                       test_data, cmap=cmap, norm=norm)
        ax1.pcolormesh(self.lat_SH_array, self.theta_domain_array, -
                       test_data, cmap=cmap, norm=norm)
        cs1 = ax1.contour(lat_array, theta_lev_array,
                          output_plotting['NH', 'ipv_zonal_0'], levels=np.arange(2, 3, 1),
                          colors='red')
        cs2 = ax1.contour(lat_array, theta_lev_array,
                          output_plotting['NH', 'ipv_zonal_0'],
                          levels=np.arange(-2, -1, 1), colors='red')
        plt.plot(output_plotting['NH', 'phi_2PV'], output_plotting['NH', 'theta_2PV'],
                 marker='x', c='k', linestyle=' ', markersize=10)
        plt.plot(output_plotting['NH', 'dThdlat_lat'][0],
                 output_plotting['NH', 'dThdlat_theta'], marker='x', c='red',
                 linestyle=' ', markersize=10)
        plt.ylabel('Potential Temperature')
        ...
        theta_array_interp = self.theta_interp[
            :, np.newaxis] + np.zeros(array_shape_interp)
        ...
        bounds = np.arange(-20, 22, 2.0)
        norm = mpl.colors.BoundaryNorm(bounds, cmap.N)
        ax1.pcolormesh(lat_array_interp_NH, theta_array_interp,
                       output_plotting['NH', 'ipv_zonal_interp_0'], cmap=cmap, norm=norm)
        ax1.pcolormesh(lat_array_interp_SH, theta_array_interp,
                       output_plotting['SH', 'ipv_zonal_interp_0'], cmap=cmap, norm=norm)
        plt.plot(output_plotting['NH', 'phi_2PV'], output_plotting['NH', 'theta_2PV'],
                 marker='x', c='k', linestyle=' ', markersize=10)
        plt.plot(output_plotting['NH', 'dThdlat_lat'][0],
                 output_plotting['NH', 'dThdlat_theta'], marker='x', c='red',
                 linestyle=' ', markersize=10)
        plt.plot(output_plotting['SH', 'phi_2PV'], output_plotting['SH', 'theta_2PV'],
                 marker='x', c='k', linestyle=' ', markersize=10)
        plt.plot(output_plotting['SH', 'dThdlat_lat'][0],
                 output_plotting['SH', 'dThdlat_theta'], marker='x', c='red',
                 linestyle=' ', markersize=10)
        cs1 = ax1.contour(lat_array_interp_NH, theta_array_interp,
                          output_plotting['NH', 'ipv_zonal_interp_0'],
                          levels=np.arange(2, 3, 1), colors='red')
        cs2 = ax1.contour(lat_array_interp_SH, theta_array_interp,
                          output_plotting['SH', 'ipv_zonal_interp_0'],
                          levels=np.arange(-2, -1, 1), colors='red')
        plt.ylabel('Potential Temperature')
    ...
    def PlotPV(self):
        ...
        CS = plt.contourf(self.lat, self.p, self.PV_zonal,
                          levels=lev, cmap=cmap, norm=norm)
